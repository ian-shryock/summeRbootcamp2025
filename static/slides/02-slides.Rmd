...
We can coerce `mixed_vec` to be numeric using `as.numeric()`, but notice what happens to the character values `r emoji::emoji("eyes")`
...
Notice that `[[1]]`, `[[2]]`, and `[[3]]`, the element indices, have been replaced by the names `name`, `height` and `grammy` `r emoji::emoji("eyes")`
